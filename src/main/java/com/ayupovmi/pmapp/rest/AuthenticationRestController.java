...
import com.ayupovmi.pmapp.security.JwtTokenProvider;
...
public class AuthenticationRestController {
    ...
    public ResponseEntity<Object> authenticate (@RequestBody AuthenticationRequestDto request) {
        try {
            ...
            UserEntity userEntity = userRepository.findByEmail(request.getEmail()).orElseThrow(() ->new UsernameNotFoundException("User doesn't exist"));
            String token = jwtTokenProvider.createToken(request.getEmail(), userEntity.getRoleForSecurity().name());
